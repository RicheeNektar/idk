		...
		if (args.length >= 1) {
			if (source.getWorld().getName().equals("ASkyBlock") || source.getWorld().getName().equals("skyblock")) {
				...
				for(Player p : names) {
				    ...
				    p.sendMessage("["+ ChatColor.GREEN +"Hilfe"+ ChatColor.WHITE +"]"+ ChatColor.GOLD +""+ fulltext + " "+ ChatColor.WHITE +"("+ ChatColor.RED +""+ source.getDisplayName() +""+ ChatColor.WHITE +")");
				}
			...
			source.sendMessage(ChatColor.DARK_RED + "Fehler:"+ ChatColor.WHITE + " Hilfetext fehlt!");
		}
		return false;
	}
